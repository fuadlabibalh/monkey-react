...
function getBasename2(routers, isPermit, levelOrLevel) {
    ...
    checkinRouterDef && !isPermit &&
        result.push({
            path: routePath,
            element: ((0, jsx_runtime_1.jsx)(react_router_dom_1.Navigate, { to: baserouters?.index ? `/${baserouters?.authPath}` : `/${baserouters?.basename}/${baserouters?.authPath}` })),
        });
    !checkinRouterDef &&
        result.push({
            path: routePath,
...
function getNotFoundRoute(result, idxPath, baserouters) {
    return result.find((e) => e.path === idxPath)?.element || ((0, jsx_runtime_1.jsx)(react_router_dom_1.Navigate, { to: baserouters?.index ? `/${baserouters?.authPath}` : `/${baserouters?.basename}/${baserouters?.authPath}` }));
}
