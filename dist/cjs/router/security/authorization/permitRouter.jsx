...
exports.getBasename2 = getBasename2;
...
function getBasename(routers, isPermit, levelOrLevel) {
    ...
    for (let i = 0; i < tempRouters?.length; i++) {
        ...
        if (!isPermit &&
            ...
            temp = {
                index: true,
                element: (<react_router_dom_1.Navigate to={`/${baserouters?.basename}/${baserouters?.authPath}`}/>),
            };
            result.push({ ...temp });
        }
    ...
    let notFoundRoute = result.find((e) => e.path === `/${baserouters?.basename}/*`)?.element || (<react_router_dom_1.Navigate to={`/${baserouters?.basename}/${baserouters?.authPath}`}/>);
...
function getBasename2(routers, isPermit, levelOrLevel) {
    // checking cannot use char "/" for define default path
    let [_, basename] = window.location.pathname?.split("/");
    // check browser location for get sub path and matching with basename
    ...
    checkinRouterDef && !isPermit &&
        result.push({
            path: routePath,
            element: (<react_router_dom_1.Navigate to={baserouters?.index ? `/${baserouters?.authPath}` : `/${baserouters?.basename}/${baserouters?.authPath}`}/>),
        });
    !checkinRouterDef &&
        result.push({
            path: routePath,
...
function getNotFoundRoute(result, idxPath, baserouters) {
    return result.find((e) => e.path === idxPath)?.element || (<react_router_dom_1.Navigate to={baserouters?.index ? `/${baserouters?.authPath}` : `/${baserouters?.basename}/${baserouters?.authPath}`}/>);
}
